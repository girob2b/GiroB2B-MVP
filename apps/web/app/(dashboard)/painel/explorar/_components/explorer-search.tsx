...
import {
  ...
  ChevronDown, ClipboardList, Clock, TrendingUp,
} from "lucide-react";
...
const RECENT_SEARCHES_KEY = "girob2b_recent_searches";
...
function saveRecentSearch(term: string) {
  ...
  const current = loadRecentSearches().filter(t => t.toLowerCase() !== trimmed.toLowerCase());
  const next = [trimmed, ...current].slice(0, RECENT_SEARCHES_MAX);
  try {
...
export default function ExplorerSearch() {
  ...
  // Query "submetida" = o termo que está rodando na busca. Diferente de
  // `filters.query` (valor que o user digita). Só atualiza quando o user
  // dispara explicitamente (Enter ou clique numa sugestão do autocomplete).
  // Princípio: enquanto digita, ele vê sugestões; ao apertar Enter, busca.
  ...
  const isIdleState = !submittedQuery.trim() && !filters.category && !filters.state && filters.moqIndex === 0 && filters.supplierTypes.length === 0;

  // Filtra sugestões enquanto o user digita. Mostra recentes e populares
  // que casam com o que ele tá digitando. Se input vazio, mostra o top de cada.
  const queryLower = filters.query.trim().toLowerCase();
  const filteredRecent = queryLower
  ...
  return (
    <>
      ...
      <div className="space-y-2 mb-5">
        {/* Input + dropdown de autocomplete */}
        <div className="relative" ref={searchBoxRef}>
          ...
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                e.preventDefault();
                submitQuery(filters.query);
              } else if (e.key === "Escape") {
                setSearchFocused(false);
              }
            }}
            placeholder="Buscar produtos ou fornecedores e pressione Enter"
          ...
          {filters.query && (
            <button
              onClick={() => { updateFilters({ query: "" }); submitQuery(""); }}
          ...
          {/* Dropdown de autocomplete — pesquisas anteriores + populares */}
          {showSuggestions && (
            <div className="absolute top-full left-0 right-0 z-30 mt-2 rounded-2xl border border-slate-200 bg-white shadow-lg overflow-hidden">
      ...
      {loading && !error && (
        <div className="rounded-2xl border border-slate-200 bg-white py-4">
          <GiroLoader variant="inline" label="Buscando…" />
        </div>
      )}
      ...
      {!loading && !error && (
        <>
          {results.length === 0 ? (
            !submittedQuery.trim() && !filters.category && !filters.state ? (
              ...
              <div className="flex flex-col items-center justify-center gap-5 rounded-3xl border border-slate-200 bg-white px-6 py-12 text-center shadow-sm">
                ...
                <div className="max-w-md space-y-1">
                  ...
                  <p className="text-sm text-slate-500">
                    {submittedQuery.trim()
                      ? <>Não achamos fornecedores para <strong className="text-slate-700">&ldquo;{submittedQuery}&rdquo;</strong>. Peça para nossos admins cadastrarem — leva 1 a 2 dias úteis.</>
