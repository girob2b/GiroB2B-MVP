...
function getSaoPauloDayStart() {
  ...
  return new Date(`${formatted}T00:00:00-03:00`);
}
...
async function ensureSupplier(
  ...
  if (supplierError || !supplier) {
    throw new InquiryValidationError("Fornecedor nao encontrado.", 404);
  }

  if (supplier.suspended) {
    throw new InquiryValidationError("Fornecedor indisponivel para contato.", 422);
  }
...
async function getOrCreateBuyer(
  ...
  if (existing) {
    ...
    if (Object.keys(updatePayload).length > 0) {
      ...
      if (updateError) {
        throw new InquiryValidationError("Nao foi possivel atualizar o perfil de comprador.", 500);
      }
    }
...
async function enqueueInquiryNotification(supplier: SupplierLookup, inquiryId: string) {
  ...
  try {
    ...
    // Ignore while notifications table is not fully available in every environment.
  }
}
...
export async function createDirectedInquiry(
  ...
  const startOfDay = getSaoPauloDayStart().toISOString();
  const dedupKey = buildDedupKey(buyer.id, supplier.id, input.product_id ?? null);
  const dedupSince = new Date(Date.now() - INQUIRY_DEDUP_HOURS * 60 * 60 * 1000).toISOString();
  ...
  if (!result.deduplicated) {
    await enqueueInquiryNotification(supplier, result.inquiry.id).catch(() => {
      // Notification plumbing is best-effort at this stage.
    });
  }
...
export async function listBuyerInquiries(userId: string, token: string) {
  ...
  const { data, error } = await supabase
    ...
    .select(`
      ...
      contact_unlocked,
      created_at,
      suppliers(id, trade_name, slug, plan)
    `)
    .eq("buyer_id", buyer.id)
    .order("created_at", { ascending: false });

  if (error) {
    throw new Error("Erro ao buscar suas cotacoes.");
  }

  return (data ?? []) as unknown as InquiryListRow[];
}

export async function listSupplierInquiries(userId: string, token: string) {
  const supabase = createClient(token);
  const { data: supplier, error: supplierError } = await supabase
    .from("suppliers")
    .select("id, plan")
    .eq("user_id", userId)
    .single<{ id: string; plan: SupplierPlan }>();
  ...
  if (error) {
    throw new Error("Erro ao buscar cotacoes recebidas.");
  }

  return ((data ?? []) as InquiryListRow[]).map(inquiry => {
    if (supplier.plan !== "free" && inquiry.contact_unlocked) {
      return inquiry;
    }
