...
export async function syncDerivedUserRole(userId: string) {
  const admin = createAdminClient();
  return syncDerivedUserRoleWithAdmin(admin, userId);
}
...
export async function syncDerivedUserRoleWithAdmin(
  ...
  if (profileError) {
    throw new Error("Nao foi possivel sincronizar o papel do usuario.");
  }
